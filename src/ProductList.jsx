...
function ProductList({ onHomeClick }) {
    ...
    const countCarQuantity = () => {
        return cartItems.reduce(function (acc, item) { return acc + item.quantity; }, 0);
    }

    const plantQuantity = (plantName) => {
        const plant = cartItems.find(item => item.name === plantName);
                ...
                <div style={styleObjUl}>
                    ...
                    <div> 
                        <a href="#" onClick={(e) => handleCartClick(e)} style={styleA}>
                            ...
                            <text className='cart_quantity_count'>{countCarQuantity()}</text>
                            </h1>
                        </a>
                    </div>
                </div>
                  ...
                  <div className="product-list"> {/* Container for the list of plant cards */}
                    {category.plants.map((plant, plantIndex) => ( // Loop through each plant in the current category
                      <div className="product-card" key={plantIndex}> {/* Unique key for each plant card */}
                        ...
                        <button
                            className={plantQuantity(plant.name) === 0 ? "product-button" : "product-button added-to-cart btn-disable"}
                            onClick={() => handleAddToCart(plant)} // Handle adding plant to cart
                        >
                          {plantQuantity(plant.name) === 0 ? "Add" : "Added"} to Cart
                        </button>
                      </div>
                    ))}
                  </div>
